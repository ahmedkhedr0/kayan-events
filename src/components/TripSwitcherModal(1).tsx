...
const AddonsBuilder: React.FC<AddonsBuilderProps> = ({ addons = [], onChange }) => {
  ...
  return (
    <div className="space-y-3 pt-2 border-t border-slate-800">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 border-b border-slate-800/80 pb-2.5">
        <div>
          <label className="block text-amber-300 font-bold text-xs flex items-center gap-1.5">
            ...
            <span>قائمة الخدمات والإضافات المخصصة للرحلة ({addons.length} خدمة)</span>
          </label>
      ...
      {addons.length === 0 ? (
        <div className="bg-slate-950/70 border border-dashed border-slate-800 rounded-2xl p-6 text-center text-xs text-slate-400 space-y-2">
        ...
        <div className="space-y-3">
          {addons.map((addon, index) => {
            ...
            return (
              <div
                key={addon.id || index}
                className="bg-slate-950/90 border border-slate-800 hover:border-slate-700 rounded-2xl p-3.5 space-y-3 shadow-md transition-all"
              >
                ...
                <div className="flex items-center gap-2">
                  <select
                    ...
                    className="bg-slate-900 border border-slate-700 text-slate-200 text-xs font-bold rounded-xl px-2.5 py-2 focus:outline-none focus:border-amber-500 shrink-0"
                  >
                  ...
                  <input
                    ...
                    className="flex-1 bg-slate-900 border border-slate-800 text-white rounded-xl px-3 py-2 text-xs font-bold focus:outline-none focus:border-amber-500 shadow-inner"
                ...
                <div className="flex items-center justify-between flex-wrap gap-2 pt-1 border-t border-slate-900/80 text-xs">
                  <div className="flex items-center gap-2">
                  ...
                  <label className="flex items-center gap-2 cursor-pointer bg-slate-900/80 hover:bg-slate-900 px-3 py-1.5 rounded-xl border border-slate-800 transition">
                    <input
                      type="checkbox"
                    ...
                    <div className="flex items-center gap-1.5 flex-wrap">
                      ...
                      <div className="flex items-center gap-1">
                        <input
                          ...
                          placeholder={currentType === 'apparel' ? 'أضف مقاس (مثال: 4XL)' : 'أضف خيار (مثال: دجاج)'}
                          className="bg-slate-950 border border-slate-800 text-white rounded-lg px-2.5 py-1 text-xs focus:outline-none focus:border-amber-500 w-32"
                        />
                        <button
                          type="button"
  ...
  return (
    ...
    >
      <div className="bg-slate-900 border border-slate-800 rounded-2xl sm:rounded-3xl w-full max-w-4xl shadow-2xl overflow-hidden flex flex-col my-auto max-h-[92dvh] sm:max-h-[90vh] shrink-0">
        ...
        <div className="bg-slate-950/90 p-2 sm:px-5 border-b border-slate-800 flex justify-between items-center gap-2 shrink-0 flex-wrap">
          <div className="flex items-center gap-2">
            <button
            ...
            >
              ...
              <span>كافة الرحلات المتاحة ({activeTripsList.length})</span>
            </button>
          ...
          {canAccessTreasury && (
            <button
              ...
              className="bg-indigo-950 hover:bg-indigo-900 text-indigo-300 border border-indigo-700/60 px-3 py-1.5 rounded-xl text-xs font-bold flex items-center gap-1.5 transition active:scale-95 shadow-sm"
            >
        ...
        <div className="p-3 sm:p-5 overflow-y-auto space-y-4 flex-1 custom-scrollbar">
          ...
          {activeTab === 'create' && (
            <form onSubmit={handleCreateSubmit} className="space-y-4 bg-slate-950 p-4 sm:p-5 rounded-2xl border border-slate-800">
              {/* Header Title */}
                  ...
                  {TRIP_PRESETS.map((preset) => {
                    ...
                    return (
                      <button
                        ...
                        className={`p-2.5 rounded-xl border text-right transition flex flex-col justify-between text-xs group relative ${
                          isSelected
                      ...
                      >
                        ...
                        <span className="font-bold text-[11px] text-white line-clamp-1 group-hover:text-amber-300">
                          {preset.title}
                        </span>
                        <span className="text-[9px] text-slate-400 mt-0.5 line-clamp-1">{preset.badge}</span>
                      </button>
                    );
                  })}
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3.5">
                <div className="md:col-span-2">
                  ...
                  <input
                    ...
                    placeholder="مثال: رحلة 4: قرية أثينا باي - رأس سدر (الفوج الرابع)"
                    className="w-full bg-slate-900 border border-slate-800 rounded-xl px-3.5 py-2.5 text-xs font-bold text-white focus:outline-none focus:border-amber-500 shadow-inner"
                    required
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-slate-900 border border-slate-800 rounded-xl px-3.5 py-2 text-xs font-bold text-white focus:outline-none focus:border-amber-500 font-mono"
                    required
                  />
                </div>

                <div>
                  <label className="block text-xs font-bold text-slate-200 mb-1">📍 الوجهة / المكان السياحي</label>
                  <input
                    type="text"
                    ...
                    className="w-full bg-slate-900 border border-slate-800 rounded-xl px-3.5 py-2 text-xs font-bold text-white focus:outline-none focus:border-amber-500"
                    required
                  />
                </div>
              </div>
              ...
              <div className="bg-slate-900/70 p-3.5 rounded-2xl border border-slate-800 space-y-2">
                <div className="flex justify-between items-center flex-wrap gap-2">
                  ...
                  <div className="flex items-center gap-1 text-xs">
                    <span className="text-[10px] text-slate-400 ml-1">تحديد سريع:</span>
                    {[
                      { buses: 2, seats: 100, label: '2 باص (100)' },
                      { buses: 3, seats: 150, label: '3 باص (150)' },
                      { buses: 4, seats: 200, label: '4 باص (200)' },
                      { buses: 6, seats: 300, label: '6 باص (300)' },
                    ].map((opt) => (
                      <button
                        key={opt.seats}
                        type="button"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-1.5 text-xs font-mono font-bold text-cyan-300 focus:outline-none focus:border-cyan-500"
                      required
                    />
                  </div>

                  <div>
                    <label className="block text-[11px] text-slate-400 font-bold mb-1">📍 نقطة التجمع الرئيسية</label>
                    <input
                      type="text"
                      value={newTripForm.assemblyLocation}
                      onChange={(e) => setNewTripForm({ ...newTripForm, assemblyLocation: e.target.value })}
                      placeholder="مثال: ميدان الرماية - أمام البوابة"
                      className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-1.5 text-xs font-bold text-white focus:outline-none focus:border-cyan-500"
                    />
                  </div>
              ...
              <div className="bg-slate-900/70 border border-amber-500/30 rounded-2xl p-3.5 space-y-3">
                <div className="flex justify-between items-center border-b border-amber-500/20 pb-2">
                  <h5 className="text-xs font-bold text-amber-400 flex items-center gap-1.5">
                    <BadgeDollarSign className="w-4 h-4" />
                    <span>التسعير المالي وقائمة الخدمات والإضافات (المينيو)</span>
                  </h5>
                  <span className="text-[10px] text-slate-400 font-mono">حساب تلقائي للتذكرة ⚡</span>
                </div>

                <div className="grid grid-cols-2 gap-3 text-xs">
                  <div>
                    <label className="block text-slate-200 font-bold mb-1">سعر التذكرة الأساسية (بدون إضافات) *</label>
                    <div className="relative">
                      <input
                        ...
                        className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 font-mono font-bold text-amber-300 focus:outline-none focus:border-amber-500 pl-10"
                        required
                      />
                  ...
                  <div>
                    <label className="block text-slate-200 font-bold mb-1">العربون المبدئي الافتراضي للتثبيت</label>
                    <div className="relative">
                      <input
                        type="number"
                        min={0}
              ...
              <div className="bg-gradient-to-r from-emerald-950/40 via-slate-900 to-slate-950 p-3.5 rounded-2xl border border-emerald-500/30 flex flex-col sm:flex-row items-center justify-between gap-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <span className="text-xs font-bold text-emerald-300 block">
                      حاسبة الجدوى والإيرادات التقديرية للرحلة 📈
                    </span>
                    <span className="text-[11px] text-slate-400">
                      بناءً على اكتمال سعة ({newTripForm.totalSeats} مشترك) × {newTripForm.ticketPrice} ج.م تذكرة
                    </span>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-4 text-xs font-mono shrink-0">
                  <div className="text-center">
                    <span className="text-[10px] text-slate-400 block font-sans">دخل التذاكر</span>
                    <span className="font-bold text-white">{estimatedTicketRevenue.toLocaleString()} ج.م</span>
                  </div>
                  <div className="text-center">
                    <span className="text-[10px] text-slate-400 block font-sans">دخل الإضافات</span>
                    <span className="font-bold text-amber-300">+{estimatedAddonRevenue.toLocaleString()} ج.م</span>
                  </div>
                  <div className="text-center bg-emerald-500/20 px-3 py-1.5 rounded-xl border border-emerald-500/40">
                    <span className="text-[10px] text-emerald-400 block font-sans font-bold">الإجمالي المتوقع</span>
                    <span className="font-black text-emerald-300 text-sm">{totalEstimatedRevenue.toLocaleString()} ج.م</span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex justify-end gap-3 pt-3 border-t border-slate-800">
                <button
                  type="button"
                  onClick={() => setActiveTab('list')}
                  className="bg-slate-900 hover:bg-slate-800 text-slate-300 px-5 py-2.5 rounded-xl text-xs font-bold border border-slate-800 transition"
                >
                  إلغاء
                </button>
                <button
                  type="submit"
                  className="bg-gradient-to-r from-amber-500 to-amber-400 hover:from-amber-400 hover:to-amber-300 text-slate-950 font-black px-7 py-2.5 rounded-xl text-xs flex items-center gap-2 transition shadow-xl shadow-amber-500/25 active:scale-95"
                >
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl sm:rounded-3xl w-full max-w-2xl max-h-[92dvh] sm:max-h-[90vh] flex flex-col overflow-hidden shadow-2xl my-auto shrink-0">
            ...
            <form onSubmit={handleEditSubmit} className="flex flex-col flex-1 overflow-hidden">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <div className="md:col-span-2">
                    ...
                    <input
                      ...
                      className="w-full bg-slate-950 border border-slate-800 text-white rounded-xl p-2.5 font-bold focus:border-amber-500 focus:outline-none"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-slate-950 border border-slate-800 text-white rounded-xl p-2.5 font-mono focus:border-amber-500 focus:outline-none"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-slate-950 border border-slate-800 text-amber-300 rounded-xl p-2.5 font-mono font-bold focus:border-amber-500 focus:outline-none"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-slate-950 border border-slate-800 text-white rounded-xl p-2.5 font-mono text-xs focus:border-amber-500 focus:outline-none"
                    />
                  </div>
              ...
              <div className="flex justify-between sm:justify-end items-center gap-2 p-3 sm:p-4 bg-slate-950 border-t border-slate-800 shrink-0">
                ...
                <button
                  type="submit"
                  className="bg-amber-500 hover:bg-amber-400 text-slate-950 font-black px-6 py-2 rounded-xl text-xs shadow-lg shadow-amber-500/20 transition active:scale-95"
                >
                  حفظ التعديلات
                </button>
              </div>
