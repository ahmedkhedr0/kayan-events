  ...
  const handleDownloadHDImage = async () => {
    ...
    try {
      ...
      if (result.success && result.dataUrl) {
        ...
        // If on mobile device, open preview modal immediately or attempt web share for saving to Photos
    ...
    try {
      ...
      if (elem && typeof window !== 'undefined') {
        ...
        if (blob) {
          ...
          // 1. Mobile Web Share API: Shares PNG image file directly to WhatsApp app
          if (navigator.share && navigator.canShare && navigator.canShare({ files: [file] })) {
            await navigator.share({
              title: `تذكرة ${student.name}`,
              text: `تذكرة حجز رقمية معتمدة #${student.ticketCode} - ${settings.companyNameAr || 'شركة كيان'}`,
              files: [file],
            });
            setIsExporting(false);
            return;
      ...
      await exportTicketElementAsPNG(elementId, `TICKET_${student.ticketCode}_${student.name}`);
          ...
          <div className="bg-slate-900 border border-amber-500/40 rounded-3xl p-5 max-w-3xl w-full space-y-4 text-center shadow-2xl relative">
            ...
            <div className="flex items-center justify-between flex-wrap gap-2 text-xs">
              ...
              <a
                ...
                download={`KAYAN_Ticket_${student.ticketCode}_${student.name}.png`}
                className="bg-amber-600 hover:bg-amber-500 text-white font-black px-4 py-2 rounded-xl flex items-center gap-1.5 shadow"
              >
