...
export const AuthLockScreen: React.FC<AuthLockScreenProps> = ({
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col justify-center items-center p-3 sm:p-6 relative overflow-hidden selection:bg-amber-500 selection:text-slate-950 font-sans pt-16 sm:pt-6">
      ...
      {/* Top Header Bar with Minimalist Install App Button & Status */}
      <header className="absolute top-0 left-0 right-0 z-30 p-3 sm:p-4 flex items-center justify-between max-w-4xl mx-auto w-full">
        {/* System Version Badge */}
        <div className="flex items-center gap-1.5 bg-slate-900/70 backdrop-blur-md px-2.5 py-1 rounded-full border border-slate-800 text-[10px] text-slate-400">
          <span className="w-1.5 h-1.5 rounded-full bg-emerald-400"></span>
          <span className="font-mono font-medium tracking-wide">KAYAN OS</span>
        </div>

        {/* Sleek Minimalist Install App Button */}
        {onInstallPWA && (
          <button
            type="button"
            onClick={onInstallPWA}
            className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-slate-900/80 hover:bg-slate-800 text-slate-300 hover:text-amber-300 border border-slate-700/70 hover:border-amber-500/40 text-[11px] font-medium transition-all duration-200 shadow-sm active:scale-95 cursor-pointer backdrop-blur-md"
            title="Install Kayan App (PWA)"
          >
