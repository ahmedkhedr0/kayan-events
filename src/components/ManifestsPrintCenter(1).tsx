  ...
  const handleExportPDF = async (forcedType?: string) => {
    ...
    try {
      if (typeToExport === 'full_students') {
        ...
        await generateStudentsComprehensiveReportPDF(
          filteredStudents,
          settings,
        ...
        await generateBusManifestPDF(busParam, students, currentBusDriver || undefined, settings);
        ...
        if (tshirtSubMode === 'factory' || forcedType === 'tshirt_factory') {
          ...
          await generateTShirtFactoryPDF(students, settings, busParam);
        } else {
          setExportMessage('جاري إنشاء كشف تسليم وتوزيع التيشرتات A4...');
          await generateTShirtDistributionPDF(students, settings, busParam);
        }
        ...
        if (mealSubMode === 'kitchen' || forcedType === 'meal_kitchen') {
          ...
          await generateMealKitchenPDF(students, settings, busParam);
        } else {
          setExportMessage('جاري إنشاء كشف تسليم الوجبات الميداني A4...');
          await generateMealDistributionPDF(students, settings, busParam);
        }
        ...
        await generateFinancialManifestPDF(students, settings, busParam);
      } else {
        setExportMessage('جاري إنشاء الكشف الشامل الماستر A4...');
        await generateMasterAttendanceDeliveryPDF(students, settings, 'all');
      }
    } catch (e) {
